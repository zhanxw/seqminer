...
class GeneAnnotation{
 public:
  ...
  /// @return 0: success
  int openGeneFile(const std::string& geneFileName, const std::string& geneFormat) {
    // set foramt
    GeneFormat format;
    ...
    if (s == "refflat") {
      ...
      REprintf( "Unknown format (other than refFlat, knownGene, refGene)!\nNow quitting...\n");
      // LOG << "Input gene format is wrong!\n";      
      return -1; // abort();
    }

    // read gene file
    REprintf( "Load gene file %s...\n", geneFileName.c_str());
    std::string line;
    ...
    this->sortGene();
    REprintf( "DONE: %d gene loaded from %s.\n", totalGene, geneFileName.c_str());
    // LOG << "Gene file " << geneFileName << " loads succeed!\n";
    return 0;
  };
  void openCodonFile(const std::string& codonFileName) {
    REprintf( "Load codon file %s...\n", codonFileName.c_str());
    this->codon.open(codonFileName);
    REprintf( "DONE: codon file loaded.\n");
    // LOG << "Codon file " << codonFileName << " loads succeed!\n";
    return;
  };
  void openReferenceGenome(const std::string& referenceGenomeFileName) {
    REprintf( "Load reference genome %s...\n", referenceGenomeFileName.c_str());
    this->gs.open(referenceGenomeFileName);
    REprintf( "DONE: %d chromosomes and %ld bases are loaded.\n", this->gs.size(), this->gs.getGenomeLength());
    // LOG << "Reference genome file " << referenceGenomeFileName << " loads succeed!\n";
    return;
  };
  void openPriorityFile(const std::string& fileName) {
    REprintf( "Load priority file %s...\n", fileName.c_str());
    int ret = this->priority.open(fileName);
    REprintf( "DONE: %d priority annotation types loaded.\n", ret);
  ...
  void outputAnnotationStats(const std::string& outputFileName) {
    // output frequency files
    const std::string& fn = outputFileName;

    // output annotation frequency (all types of annotation)
    std::string ofs = fn+".anno.frq";
    this->printAnnotationFrequency(ofs.c_str());
    REprintf( "DONE: Generated frequency of each annotype type in [ %s ].\n", ofs.c_str());
    ...
    REprintf( "DONE: Generated frequency of each highest priority annotation type in [ %s ].\n", ofs.c_str());
    ...
    REprintf( "DONE: Generated frequency of each base change in [ %s ].\n", ofs.c_str());
    ...
    REprintf( "DONE: Generated frequency of each codon change in [ %s ].\n", ofs.c_str());
    ...
    REprintf( "DONE: Generated frequency of indel length in [ %s ].\n", ofs.c_str());
...
public:
  ...
  void printTsTvRatio(){
    ...
    if (tv != 0) {
      ...
      REprintf( "Ts/Tv ratio: %g\n", tstv);
    } else {
      // LOG << "Ts/Tv ratio: NA\n";
      REprintf( "Ts/Tv ratio: NA\n");
    }
    // LOG << "Ts observed: " << ts << " times; Tv observed: " << tv << " times.\n";
    REprintf( "Ts observed: %d  times; Tv observed: %d times.\n", ts, tv);
  };
